...
def heuristic_scan(content):
    found = {'sqs': set(), 'sns': set(), 's3': set(), 'lambda': set(), 'k8s': set(), 'redis': set(), 'other_arns': set()}
...
# تبسيط Lambda creation (أو إزالته)
def create_lambda_simple(clients, name):
    print(f"ℹ️ Lambda creation skipped: {name} (use real deployment)")
    return False
...
def run_setup(settings_path=None):
    ...
    resources = {'sqs': set(), 'sns': set(), 's3': set(), 'lambda': set(), 'k8s': set(), 'redis': set(), 'other_arns': set()}
    ...
    created = {'sqs':0,'sns':0,'s3':0,'lambda':0,'k8s':0,'redis':0}
